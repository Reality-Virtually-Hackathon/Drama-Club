...
//        }
    ...
    private void OnError(string error)
    {
        ...
        Log.Debug("VoiceProcessor", "Error! {0}", error);
    }
        ...
        if (result != null && result.results.Length > 0)
        {
            foreach (var res in result.results)
            {
                ...
                if (res.keywords_result != null && res.keywords_result.keyword != null)
                {
                    ...
                    {
                        Log.Debug("VoiceProcessor", "keyword: {0}, confidence: {1}, start time: {2}, end time: {3}", keyword.normalized_text, keyword.confidence, keyword.start_time, keyword.end_time);
                    }
                }
            }
        }
